...
class CascadeParallelEncoderDecoder(ParallelEncoderDecoder):
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        pred_l_list.append(prev_output)
        for i in range(1, self.num_stages):
            cur_output = self.decode_head_l[i].forward(x_l, prev_output)
            pred_l_list.append(cur_output)
        ...
        if self.train_cfg.get('consistency_w_hard_label', False):
            ...
            one_hot_label_l[max_pred_l < confidense_threshold] = ignore_index
            ...
            # sharpe_label_l = F.softmax(torch.pow(pred_l_list[-1], T), 1)
            # sharpe_label_r = F.softmax(torch.pow(pred_r_list[-1], T), 1)
            sharpe_label_l = F.normalize(torch.pow(pred_l, T), dim=1, p=1)
            sharpe_label_r = F.normalize(torch.pow(pred_r, T), dim=1, p=1)
            if self.train_cfg.get('auxiliary_consistency', False):
                aux_pred_l = F.softmax(pred_l_list[-2], 1)
                aux_pred_r = F.softmax(pred_r_list[-2], 1)
                # sharpe_aux_l = F.softmax(torch.pow(pred_l_list[-2], T), 1)
                # sharpe_aux_r = F.softmax(torch.pow(pred_r_list[-2], T), 1)
